...
import { calculateScheduledUpdateTime, isUpdateAllowed, queuePendingUpdate, registerPostSchedule } from './services/updateScheduler.js';
...
async function runMockSimulation() {
  ...
  for (const stepState of MOCK_MATCH_LIFECYCLE) {
    ...
    // Process edits with 2–5 min delay protection
    ...
    // Process any previously held pending updates whose 2–5 min delay has elapsed
    for (const postRecord of Object.values(canonicalFacebookPosts)) {
      ...
      if (isUpdateAllowed(postRecord)) {
        ...
        const updatedMsg = formatEventPost(pending.event, stepState);
        const success = await mockFacebook.updatePagePost(postRecord.postId, updatedMsg);
        ...
        if (success) {
          ...
          console.log(`\n✏️ [FACEBOOK EVENT POST UPDATED (SCHEDULED): ${pending.event.type}] (${pending.eventId})`);
          console.log(updatedMsg);
          console.log(`════════════════════════════════════════════════════`);
