...
import { calculateScheduledUpdateTime } from './updateScheduler.js';
 ...
 * @returns {Promise<any>}
 */
export async function recordFacebookPost(fixtureId, postId, eventId, type, extraMeta = {}) {
  return updateMatchRecord(fixtureId, (match) => {
    if (!match.facebookPosts) match.facebookPosts = {};
    const createdAt = extraMeta.createdAt || match.facebookPosts[postId]?.createdAt || new Date().toISOString();
    const earliestUpdateAt =
      extraMeta.earliestUpdateAt ||
      match.facebookPosts[postId]?.earliestUpdateAt ||
      calculateScheduledUpdateTime(createdAt);
